    ...
    m_s11SapMme (0)
{
  ...
  m_s11SapSgw = new MemberEpcSRSapSgw<EpcSgwPgwApplication> (this);
}
...
void 
EpcSgwPgwApplication::SetS11SapMme (EpcSRSapMme * s)
{
  m_s11SapMme = s;
}
...
void 
EpcSgwPgwApplication::DoCreateSessionRequest (EpcSRSapSgw::CreateSessionRequestMessage req)
{
  ...
  EpcSRSapMme::CreateSessionResponseMessage res;
  res.teid = req.imsi; // trick to avoid the need for allocating TEIDs on the S11 interface

  for (std::list<EpcSRSapSgw::BearerContextToBeCreated>::iterator bit = req.bearerContextsToBeCreated.begin ();
       ...
       ++bit)
    {
      ...
      EpcSRSapMme::BearerContextCreated bearerContext;
...
void 
EpcSgwPgwApplication::DoModifyBearerRequest (EpcSRSapSgw::ModifyBearerRequestMessage req)
{
  ...
  EpcSRSapMme::ModifyBearerResponseMessage res;
  res.teid = imsi; // trick to avoid the need for allocating TEIDs on the S11 interface
  res.cause = EpcSRSapMme::ModifyBearerResponseMessage::REQUEST_ACCEPTED;
  m_s11SapMme->ModifyBearerResponse (res);
}
 
void
EpcSgwPgwApplication::DoDeleteBearerCommand (EpcSRSapSgw::DeleteBearerCommandMessage req)
{
  NS_LOG_FUNCTION (this << req.teid);
  uint64_t imsi = req.teid; // trick to avoid the need for allocating TEIDs on the S11 interface
  std::map<uint64_t, Ptr<UeInfo> >::iterator ueit = m_ueInfoByImsiMap.find (imsi);
  ...
  EpcSRSapMme::DeleteBearerRequestMessage res;
  res.teid = imsi;

  for (std::list<EpcSRSapSgw::BearerContextToBeRemoved>::iterator bit = req.bearerContextsToBeRemoved.begin ();
       bit != req.bearerContextsToBeRemoved.end ();
       ++bit)
    {
      EpcSRSapMme::BearerContextRemoved bearerContext;
      bearerContext.epsBearerId =  bit->epsBearerId;
...
void
EpcSgwPgwApplication::DoDeleteBearerResponse (EpcSRSapSgw::DeleteBearerResponseMessage req)
{
  NS_LOG_FUNCTION (this << req.teid);
  uint64_t imsi = req.teid; // trick to avoid the need for allocating TEIDs on the S11 interface
  std::map<uint64_t, Ptr<UeInfo> >::iterator ueit = m_ueInfoByImsiMap.find (imsi);
  ...
  for (std::list<EpcSRSapSgw::BearerContextRemovedSgwPgw>::iterator bit = req.bearerContextsRemoved.begin ();
